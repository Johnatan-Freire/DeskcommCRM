-- 0169: configuração por organização da integração com o sistema escolar
...
--
-- Sem tela de configuração ainda (deferido) — por isso o SELECT de
-- `authenticated` já nasce revogado, mesma razão do apêndice de
-- ai_provider_credentials: nenhum caminho de browser precisa ler
-- api_key_encrypted/iv/tag, e não expor por padrão é mais barato que expor
-- e revogar depois.
