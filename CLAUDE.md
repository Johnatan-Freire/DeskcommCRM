...
- **A IA só atende mensagem que aconteceu DEPOIS de ser ligada** (migration 0403). `ai_agents.service_enabled_at` é "desde quando este agente pode responder": gravada **só por trigger** (`trg_ai_agents_inicio_do_atendimento`) — publicar, despausar ou trocar de modo carimba `now()`; sair do ar zera; continuar no ar preserva (a aplicação não consegue movê-la). A decisão é SQL, para os dois transportes: `fn_ia_pode_responder_mensagem(org, mensagem, agente?)` compara o `sent_at` REAL contra conexão e ativação, fronteira inclusiva, **fail-closed** (horário no futuro, ativação desconhecida, nenhum agente no ar → não dispara). Consultada no drain (qualquer agente no ar do número) e de novo no `inbound_turn` com o agente que o roteador resolveu — é o que barra backlog da pausa, replay, redrive e job represado. Turno sem agente resolvido não responde mais pelo "genérico": sem ativação não há corte a medir
