...
async function handle(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    //
    // Mora AQUI, e não numa rota de cron própria, por packaging: o
    ...
    // instalado sem ninguém editar nada. Nome e cadência também servem:
    // retenção é remover dado pessoal no prazo, e a LGPD dá D+15.
    //
    // O client aqui é o de SERVICE ROLE, que bypassa a RLS — por isso
    // `completarRedacaoDoContato` filtra `organization_id` à mão em toda
    // query, com a org vinda da própria linha de `contacts` (fonte confiável).
  ...
  // Uma linha POR CONTATO, na org dele: é a auditoria que responde ao titular,
  // e uma linha global `retention.sweep_run` não responde a ninguém em
  // particular. Só para quem TINHA resíduo — `completados` já é a lista
  // filtrada por `houveRedacao`, e o `if` deixa isso explícito para o guarda
  // de AST que varre `app/api/v1/cron/` (`cron-audita-so-quando-ha-efeito`):
  // ele não conta `for` como condição, e está certo em não contar.
