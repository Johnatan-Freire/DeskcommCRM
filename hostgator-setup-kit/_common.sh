...
# contém "email.eq.", e a resposta é SEMPRE vazia. Medido:
#
...
# instalação sem SMTP, que é o estado normal de um self-host. Achado ao triar o
# CHANGELOG do upstream (melgarafael/DeskcommCRM v1.14.0) — mesmo bug, mesma
# causa, e nosso kit tinha exatamente essa versão pré-conserto.
#
...
# Este arquivo roda sob `set -euo pipefail`, e o consumidor resolve o UUID numa
...
# o caso em que a mensagem existe para falar. Sem isto, o operador que erra uma
# letra no endereço não vê aviso nenhum, só o prompt de volta.
owner_id_by_email() {
