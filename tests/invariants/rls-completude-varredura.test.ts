...
const PROVA_PROPRIA: readonly Excecao[] = [
  ...
  { tabela: "org_sistema_escolar_config", razao: "tests/invariants/sistema-escolar-config-acl.test.ts — credencial exclusiva do servidor (SELECT revogado de anon/authenticated, ACL e RLS verificadas); UPDATE/DELETE por authenticated são recusados por ACL até na própria organização (SELECT ausente bloqueia o WHERE antes da RLS); a única escrita alcançável é INSERT cego, e a with-check confina: admin da organização A não cria linha para B (controle negativo) e cria na própria org nova (controle positivo, prova que a policy não é deny-all)." },
