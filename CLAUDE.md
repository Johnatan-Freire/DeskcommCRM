...
- **O agente nunca responde mensagem de antes de conectar.** WAHA/NOWEB sincroniza histórico do WhatsApp ao parear um número novo, e o mesmo webhook que entrega mensagem NOVA entrega a ANTIGA também, sem nada que distinga as duas. `channel_sessions.first_connected_at` (migration 0398, trigger `fn_marcar_primeira_conexao`, `coalesce` protege de reconexão empurrar a data) grava o instante em que a sessão ficou `WORKING` pela PRIMEIRA vez; o drain do agente consulta esse corte via `fn_ia_pode_responder_mensagem` (ver o item seguinte) e `workers/ai-response-worker.ts` compara `messages.sent_at` (horário REAL do WhatsApp) direto, antes de despachar — mensagem mais antiga é gravada no CRM normalmente, só não gera resposta automática. `NULL` (sessão já `WORKING` antes desta migration) não corta nada, de propósito: sem instante real de conexão passada para comparar, o comportamento de quem já atende hoje não muda
- **A IA só atende mensagem que aconteceu DEPOIS de ser ligada** (migration 0403). `ai_agents.service_enabled_at` é "desde quando este agente pode responder": gravada **só por trigger** (`trg_ai_agents_inicio_do_atendimento`) — publicar, despausar ou trocar de modo carimba `now()`; sair do ar zera; continuar no ar preserva (a aplicação não consegue movê-la). A decisão é SQL, para os dois transportes: `fn_ia_pode_responder_mensagem(org, mensagem, agente?)` compara o `sent_at` REAL contra conexão e ativação, fronteira inclusiva, **fail-closed** (horário no futuro, ativação desconhecida, nenhum agente no ar → não dispara). Consultada no drain (qualquer agente no ar do número), de novo no `inbound_turn` com o agente que o roteador resolveu, e no worker de sentimento (cujo aviso de handoff é IA falando com o lead) — é o que barra backlog da pausa, replay, redrive e job represado. Turno sem agente resolvido não responde mais pelo "genérico": sem ativação não há corte a medir
  - **Follow-up de silêncio só reengaja o atendimento AUTOMÁTICO esperando o lead** — `fn_silencio_pode_reengajar` (no sweep) e `fn_followup_pode_enviar` (no instante do envio, nos dois emissores): quem falou por último tem de ser `ai`/`automation`, sem humano/handoff/`force_human`, conversa aberta, agente no ar, último inbound depois da ativação, um reengajamento por silêncio. Motivo: incidente de 2026-09-26 — fluxo só de texto fixo (que não exige agente) cobrou conversa encerrada por humano com o agente pausado
  - **A fila de reenvio tem validade:** `redriveQueued` marca `failed`/`fila_expirada` a mensagem automática `queued` há mais de 30 min (`REDRIVE_MAX_AGE_MS_PADRAO`) em vez de mandá-la na reconexão
