...
async function main(): Promise<void> {
  ...
  if (idsAntigos.length > 0) {
    // Só os runs saem. `api_audit_log` não aceita DELETE de service_role
    // (migration 0277), e não precisa: a contagem da tela conta a auditoria
    // pelo JOIN com `ai_agent_runs`, então a linha cujo run foi apagado deixa
    // de contar de qualquer forma.
