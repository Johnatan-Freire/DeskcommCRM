/**
 * A CASCATA DE ANONIMIZAÇÃO RETOMA DE ONDE PAROU.
 *
 * ─── O defeito ───────────────────────────────────────────────────────────────
 *
 * `POST /api/v1/lgpd/anonymize` tem um early-return em `existing.is_anonymized`
 * que devolvia `200 { action: "already_anonymized" }` ANTES de
 ...
 * `contacts` + `crm_leads` + `crm_lead_activities`, e entrega um terço. Achado ao
 * triar o CHANGELOG do upstream (melgarafael/DeskcommCRM v1.14.0) — mesmo defeito.
 *
