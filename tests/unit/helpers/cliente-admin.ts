 ...
 *
 * Resolver o identificador-raiz de uma cadeia `x.from(...).eq(...)` e saber
 * quais nomes, NAQUELE arquivo, vieram de `createAdminClient()` é o ponto —
 * e não um detalhe de estilo: procurar a string "createAdminClient" no
 * arquivo inteiro daria verde para um handler que tem o cliente admin numa
 * função e o de sessão na outra.
 */
import ts from "typescript";

/**
 * O identificador-raiz de uma cadeia `x.from(...).eq(...)`; `null` quando a
 * raiz não é um nome (ex.: índice de array, chamada de outra coisa).
 *
 ...
 *
 * O que fica de fora, de propósito: cliente admin recebido por PARÂMETRO (o
 * caminho de `lib/mcp/server.ts`, que entrega o cliente de serviço ao
 * handler). Atravessar arquivos é outra varredura — declarado no docstring
 * do teste que usa isto.
 */
