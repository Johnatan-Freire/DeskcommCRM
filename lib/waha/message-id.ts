 ...
 * `messages.external_id` — para achar e apagar a linha que o webhook criou
 * antes de o envio conhecer o próprio id.
 *
 ...
 * composto a partir do destinatário, porque sem ele a lista nunca contém a
 * forma que o webhook realmente gravou. `true_` porque o eco de um envio nosso
 * é sempre `fromMe`.
 *
 * Mora aqui, e não no adapter, porque tem DOIS usuários que precisam da mesma
 * resposta: o envio normal (`wahaAdapter.echoExternalIds`, chamado por
 * `app/api/v1/messages/_handler.ts`) e o reenvio do watchdog
 * (`lib/agent-engine/edge/crm/session-reconciler.ts`), que roda noutro
 * processo, com `pg` cru e sem o seam de canal. Duas cópias desta regra
 * divergem sem quebrar nada à vista: o eco some por um caminho e fica pelo
 * outro — que é como a mensagem duplicada "voltava" depois de uma reconexão.
 *
 ...
 * composto construído não casa e a duplicata fica. O conserto de raiz é
 * canonizar o id nas duas pontas.
 */
