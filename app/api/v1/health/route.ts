    ...
    const res = await withTimeout(
      fetch(`${url}/rest/v1/organizations?select=id&limit=1`, {
        headers: {
          ...
          // na lista), o ping procuraria `<outro>.organizations`, levaria
          // `404 PGRST205` e o check declararia o banco `down` — com o CRM
          ...
          // `agent.sh` usa para REVERTER a imagem. Uma atualização boa seria
          // desfeita por uma configuração de painel que o CRM não controla.
          //
          // Nenhum client do CRM declara `db.schema` (`lib/supabase/*.ts`), e o
          // default do supabase-js é `public` — então é `public` que o app usa
          // de verdade, e é o que esta sonda tem de perguntar para estar
