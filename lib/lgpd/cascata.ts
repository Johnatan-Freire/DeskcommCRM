/**
 * A CASCATA DE ANONIMIZAÇÃO — os passos 2 e 3, num lugar só.
 *
 * ─── Por que este arquivo existe ────────────────────────────────────────────
 *
 * A rota `POST /api/v1/lgpd/anonymize` sabe retomar uma cascata interrompida
 * (contato já anonimizado, leads/atividades ainda não), e isso não basta: no
 * estado exato que a retomada conserta — `is_anonymized` verdadeiro — **a tela
 * não tem botão**. `app/app/contacts/[id]/_client.tsx` troca o botão por um
 * parágrafo quando o contato já está anonimizado, e `setAnonOpen(true)` é o
 * ÚNICO caminho para o diálogo em todo o repositório. A correção existia e era
 * inalcançável. Achado ao triar o CHANGELOG do upstream
 * (melgarafael/DeskcommCRM v1.14.0/v1.22.0, issue #310) — mesmo defeito lá.
 *
 ...
 * quem conserta é o cron diário de retenção (`app/api/v1/cron/data-retention`)
 * — `varrerRedacoesIncompletas` — e a tela só relata. O botão continua servindo
 * à PRIMEIRA execução, que é o que ele sempre foi.
 *
 * ─── Por que a regra mora AQUI, e não dentro da rota ────────────────────────
 *
 * Duas bocas escrevem a mesma redação (a rota e o cron). Com a regra
 * duplicada, a próxima correção do corte do título entraria numa e não na
 * outra, e o sintoma seria títulos redigidos de dois jeitos diferentes no
 * mesmo banco — o anti-pattern nº 2 do CLAUDE.md, duplicação sem source of
 * truth declarado.
 *
 ...
 * estragaria. Com um cron diário isso deixa de ser hipótese: sem a guarda do
 * sufixo, todo título de contato anonimizado seria comido um pedaço por dia.
 *
 * Pelo mesmo motivo o passo 3 SELECIONA antes de escrever. Reescrever todas as
 * atividades do contato incondicionalmente é inofensivo numa requisição
 * avulsa, e numa varredura diária seria escrita perpétua sobre dado que já
 * está certo, com a auditoria registrando "efeito" todo santo dia.
 */
...
export interface ResultadoDaRedacao {
   ...
   * afirmava ter redigido as três mesmo quando não redigiu nenhuma.
   */
  tabelas: string[];
